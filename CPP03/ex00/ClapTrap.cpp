...
void ClapTrap::takeDamage(unsigned int amount)
{
	if (getHit() == 0)
		...
		std::cout << "\x1B[31mClapTrap " << getName() << " has lost " << amount << " hit points!\x1B[0m";
	if (getHit() <= amount && getHit())
	{
		std::cout << " \x1B[31mClapTrap " << getName() << " has died!\x1B[0m\n";
		this->Hit_points = 0;
	}
	else
	{
...
void ClapTrap::beRepaired(unsigned int amount)
{
	if (getHit() == 0)
		...
		std::cout << "\x1B[31mClapTrap " << getName() << " has no Energy! Can't Repair!\x1B[0m\n";
	else
	{
		std::cout << "\x1B[32mClapTrap " << getName() << " repairs " << amount << " hit points!\x1B[0m";
		this->Hit_points += amount;
		std::cout << " " << getHit() << " left!\n";
		this->Energy_points--;
	}
